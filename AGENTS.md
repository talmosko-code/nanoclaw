...
- Inline `#` comments in `.env` values (e.g. `OPENCODE_MODEL=provider/model   # comment`) get included verbatim in the model ID string, causing "Model not found" errors; always put comments on their own lines.
- `pino` and `pino-pretty` are directly imported in `src/whatsapp-auth.ts` and `src/channels/whatsapp.ts`; they must stay in `package.json` even when merging skill updates (e.g. `skill/native-credential-proxy`) that attempt to remove them.
