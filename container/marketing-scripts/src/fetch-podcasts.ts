...
const TRANSCRIPTION_MAX_MINUTES = parseInt(
  process.env.TRANSCRIPTION_MAX_MINUTES || '20',
  10,
);
...
function loadPodcastSources(jsonPath: string): PodcastSource[] {
  const raw = JSON.parse(
    require('fs').readFileSync(jsonPath, 'utf-8'),
  ) as Source[];
  return raw
    .filter((s) => s.type === 'podcast' && s.active !== false)
    .map((s) => ({
      name: s.name,
      url: s.url,
...
async function downloadMp3(
  ...
  try {
    ...
    if (!fileStat || fileStat.size < 2 * 1024 * 1024) {
      ...
      console.log(
        `    ✗ File too small (${fileStat ? Math.round(fileStat.size / 1024) + 'KB' : 'missing'}) — skipping`,
      );
      return null;
    }
...
function whisperAvailable(): boolean {
  ...
  const result = spawnSync('python3', ['-c', 'import faster_whisper'], {
    encoding: 'utf8',
  });
...
async function transcribeWithWhisper(mp3Path: string): Promise<string | null> {
  ...
  const cacheDir =
    process.env.WHISPER_CACHE_DIR || '/workspace/group/marketing/whisper-cache';
...
async function fetchPodcasts(options: {
  ...
  console.log(
    `Transcription: ${options.withTranscription ? 'ON (faster-whisper)' : 'OFF (metadata only)'}`,
  );
  console.log('');
  ...
  const sources = loadPodcastSources(SOURCES_PATH);
  ...
  if (options.withTranscription && !whisperAvailable()) {
    console.log(
      '⚠️  faster-whisper not available — running in metadata-only mode.',
    );
  ...
  if (options.withTranscription) {
    ...
    for (let i = 0; i < allEpisodes.length; i++) {
      ...
      console.log(
        `[${i + 1}/${allEpisodes.length}] ${episode.title.slice(0, 60)}`,
      );
  ...
  const md = allEpisodes
    .map((e) => {
      const lines = [
        `- [${e.title}](${e.url}) — ${e.podcastName} (${e.pubDate})`,
      ];
      if (e.description) lines.push(`  ${e.description.slice(0, 200)}`);
      if (e.transcript)
        lines.push(`  📝 Transcript: ${e.transcript.slice(0, 300)}...`);
      return lines.join('\n');
    })
    .join('\n\n');
