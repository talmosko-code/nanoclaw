...
export async function run(args: string[]): Promise<void> {
  ...
  if (fs.existsSync(credsPath)) {
    try {
      ...
      const parsed = JSON.parse(raw) as { registered?: boolean; account?: unknown };
      // `registered` may stay false for linked-device flows; `account` is set once linking finishes.
      const linked = parsed.registered === true || parsed.account != null;
      if (linked) {
      ...
      // Malformed creds — re-auth below after wipe
    }
    // Incomplete failed link (pairing/QR abandoned): creds exists but session never finalized.
