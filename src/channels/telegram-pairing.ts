...
export function extractAddressedText(text: string, botUsername: string): string | null {
  const trimmed = normalizePairingPlaintext(text.trim());
...
/** Strip invisible chars and map full-width digits so mobile keyboards still match. */
...
export function extractCode(text: string, botUsername: string): string | null {
  ...
  const candidate = normalizePairingPlaintext((addressed !== null ? addressed : text).trim());
