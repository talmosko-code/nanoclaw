...
/** Host DB + setup/register use `whatsapp:<jid>`; Baileys needs the bare JID. */
const WA_ROUTE_PREFIX = 'whatsapp:';

function waRoutePlatformId(sockJid: string): string {
  return sockJid.startsWith(WA_ROUTE_PREFIX) ? sockJid : `${WA_ROUTE_PREFIX}${sockJid}`;
}

function waSockJid(routeOrSock: string): string {
  return routeOrSock.startsWith(WA_ROUTE_PREFIX) ? routeOrSock.slice(WA_ROUTE_PREFIX.length) : routeOrSock;
}

/** `972…:3@s.whatsapp.net` → `972…@s.whatsapp.net` so router matches `register --platform-id`. */
function normalizePnJid(jid: string): string {
  ...
  factory: () => {
    ...
    async function resolveLidViaUsync(ws: WASocket, lidJid: string): Promise<string | undefined> {
      try {
        ...
        q.withUser(new USyncUser().withId(lidJid));
        const r = await ws.executeUSyncQuery(q);
        const id = r?.list?.[0]?.id;
        if (typeof id !== 'string') return undefined;
        const norm = jidNormalizedUser(id);
        if (!norm || !norm.endsWith('@s.whatsapp.net')) return undefined;
        const out = normalizePnJid(norm);
        const lidUser = lidJid.split('@')[0].split(':')[0];
        setLidPhoneMapping(lidUser, out);
        log.info('Resolved LID→PN via USync (chat routing)', { lidJid, pnJid: out });
        return out;
      } catch (err) {
        log.debug('USync LID→PN failed', { lidJid, err });
        return undefined;
      }
    }
        ...
        for (const msg of messages) {
          try {
            ...
            if (chatJid.endsWith('@lid') && (msg.key as any).senderPn) {
              ...
              const phoneJid = normalizePnJid(pn.includes('@') ? pn : `${pn}@s.whatsapp.net`);
            ...
            const routeId = waRoutePlatformId(chatJid);
            const pending = pendingQuestions.get(routeId);
            if (pending && content.startsWith('/')) {
              ...
              if (matched) {
                ...
                pendingQuestions.delete(routeId);
            ...
            // WhatsApp doesn't use threads — threadId is null (platformId matches setup/register rows)
            setupConfig.onInbound(routeId, null, inbound);
          } catch (err) {
    ...
    const adapter: ChannelAdapter = {
      ...
      async deliver(
        ...
        const sockJid = waSockJid(platformId);
        ...
        if (content.type === 'ask_question' && content.questionId && content.options) {
          ...
          const text = `*${title}*\n\n${question}\n\nReply with:\n${optionLines}`;
          const msgId = await sendRawMessage(sockJid, text);
          if (msgId) {
            pendingQuestions.set(waRoutePlatformId(sockJid), { questionId, options });
          ...
          try {
            await sock.sendMessage(sockJid, {
              react: {
                ...
                key: { remoteJid: sockJid, id: content.messageId as string, fromMe: false },
              },
            });
          } catch (err) {
        ...
        if (hasFiles) {
          ...
          for (const file of message.files!) {
            try {
              ...
              const sent = await sock.sendMessage(sockJid, mediaMsg);
              if (sent?.key?.id && sent.message) {
                sentMessageCache.set(sent.key.id, sent.message);
              }
        ...
        if (text) {
          ...
          return sendRawMessage(sockJid, prefixed);
        }
      },

      async setTyping(platformId: string, _threadId: string | null) {
        try {
          await sock.sendPresenceUpdate('composing', waSockJid(platformId));
        } catch (err) {
      ...
      async syncConversations(): Promise<ConversationInfo[]> {
        try {
          ...
          return Object.entries(groups)
            .filter(([, m]) => m.subject)
            .map(([jid, m]) => ({
              platformId: waRoutePlatformId(jid),
              name: m.subject,
              isGroup: true,
            }));
        } catch (err) {
